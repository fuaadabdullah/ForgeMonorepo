...
import { classifyComplexity, isProviderAvailable, routeQuery } from '../src/router/index.js'
...
describe('Router', () => {
  ...
  describe('classifyComplexity', () => {
    it('should classify simple queries', () => {
      expect(classifyComplexity('What is the capital of France?')).toBe(TaskComplexity.SIMPLE)
      expect(classifyComplexity('Define photosynthesis')).toBe(TaskComplexity.SIMPLE)
    })

    it('should classify moderate queries', () => {
      expect(classifyComplexity('What is the weather today?')).toBe(TaskComplexity.SIMPLE)
    ...
    it('should classify complex queries', () => {
      expect(classifyComplexity('Debug this complex issue')).toBe(TaskComplexity.COMPLEX)
      expect(classifyComplexity('Generate code for this feature')).toBe(TaskComplexity.COMPLEX)
    })

    it('should classify strategic queries', () => {
      expect(classifyComplexity('Plan the architecture')).toBe(TaskComplexity.STRATEGIC)
      expect(classifyComplexity('Design a comprehensive strategy')).toBe(TaskComplexity.STRATEGIC)
    })
  })
  ...
  describe('isProviderAvailable', () => {
    it('should check OpenAI availability', () => {
      ...
      expect(
        isProviderAvailable(
          { ...mockConfig, providers: { ...mockConfig.providers, openai: undefined } },
          LLMProvider.OPENAI
        )
      ).toBe(false)
    })
    ...
    it('should check Ollama availability', () => {
      ...
      expect(
        isProviderAvailable(
          { ...mockConfig, providers: { ...mockConfig.providers, ollama: undefined } },
          LLMProvider.OLLAMA
        )
      ).toBe(false)
    })
  ...
  describe('routeQuery', () => {
    ...
    it('should route with local-first strategy when Ollama available', () => {
      const localFirstConfig = {
        ...mockConfig,
        routing: { ...mockConfig.routing, strategy: RoutingStrategy.LOCAL_FIRST },
      }
    ...
    it('should handle complex queries', () => {
      const result = routeQuery('Analyze this complex dataset and provide insights', mockConfig)
