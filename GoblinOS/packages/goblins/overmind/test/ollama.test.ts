...
const mockTracingModule = vi.mocked(await import('../observability/tracing.js'))
...
describe('Ollama Provider', () => {
  ...
  describe('listOllamaModels', () => {
    ...
    it('should throw error on failure', async () => {
      ...
      await expect(listOllamaModels()).rejects.toThrow('Failed to list Ollama models: List error')
    })
  })

  describe('checkOllamaHealth', () => {
    it('should return healthy status when available', async () => {
      mockList.mockResolvedValue({ models: [{ name: 'qwen2.5:3b' }, { name: 'qwen2.5-coder:3b' }] })
  ...
  describe('generateWithOllamaStream', () => {
    it('should generate text with streaming successfully', async () => {
      ...
      const onChunk = vi.fn()
      const result = await generateWithOllamaStream('qwen2.5:3b', 'Test prompt', { onChunk })
    ...
    it('should work without onChunk callback', async () => {
      ...
      const result = await generateWithOllamaStream('qwen2.5:3b', 'Test prompt')
    ...
    it('should handle streaming errors', async () => {
      ...
      await expect(generateWithOllamaStream('qwen2.5:3b', 'Test prompt')).rejects.toThrow(
        'Ollama streaming generation failed: Streaming error'
      )
    })
    ...
    it('should include context in streaming prompt', async () => {
      ...
      await generateWithOllamaStream('qwen2.5:3b', 'Current prompt', { context })
