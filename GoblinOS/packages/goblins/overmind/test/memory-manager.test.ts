...
describe('MemoryManager', () => {
  ...
  beforeEach(() => {
    manager = createMemoryManager({
      ...
      episodes: { enabled: true, autoSummarize: true, minMessagesPerEpisode: 5, maxEpisodes: 10 },
      vectorSearch: {
        enabled: false,
  ...
  describe('Long-term memory (facts)', () => {
    it('should store and search facts', async () => {
      ...
      expect(results.some((r) => r.entry.content.includes('Docker'))).toBe(true)
    })
  ...
  describe('Entity tracking', () => {
    ...
    it('should update existing entities', async () => {
      ...
      await manager.trackEntity('Docker', 'tool', { description: 'Updated description' })
  ...
  describe('Episodic memory', () => {
    it('should create episodes', async () => {
      ...
      manager.addMessage({ role: 'assistant', content: 'Docker is a containerization platform...' })
    ...
    it('should search episodes', async () => {
      ...
      for (let i = 0; i < 5; i++) {
        manager.addMessage({ role: 'user', content: `Kubernetes question ${i}` })
        manager.addMessage({ role: 'assistant', content: `Kubernetes answer ${i}` })
      }
      await manager.createEpisode('Kubernetes Tutorial', 'Learning Kubernetes', ['kubernetes'])

      // Clear messages and add for second episode
      manager.clearConversation()
      for (let i = 0; i < 5; i++) {
        manager.addMessage({ role: 'user', content: `Docker question ${i}` })
        manager.addMessage({ role: 'assistant', content: `Docker answer ${i}` })
      }
      await manager.createEpisode('Docker Tutorial', 'Learning Docker', ['docker'])
    ...
    it('should get recent episodes', async () => {
      ...
      for (let i = 0; i < 5; i++) {
        manager.addMessage({ role: 'user', content: `Episode 1 question ${i}` })
        manager.addMessage({ role: 'assistant', content: `Episode 1 answer ${i}` })
      }
      ...
      for (let i = 0; i < 5; i++) {
        manager.addMessage({ role: 'user', content: `Episode 2 question ${i}` })
        manager.addMessage({ role: 'assistant', content: `Episode 2 answer ${i}` })
      }
