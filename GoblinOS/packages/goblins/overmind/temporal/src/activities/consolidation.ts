...
export async function identifyConsolidationCandidates(): Promise<Candidates> {
  ...
  try {
    ...
    const shortTermResponse = await axios.get(`${MEMORY_API_URL}/api/memory/short-term`)
    ...
    logger.info(`Found ${shortTermCandidates.length} short-term candidates for working memory`)

    // Fetch working memories
    const workingResponse = await axios.get(`${MEMORY_API_URL}/api/memory/working`)
    ...
    logger.info(`Found ${workingCandidates.length} working candidates for long-term memory`)
...
export async function consolidateToWorking(
  ...
  for (const memory of params.candidates) {
    try {
      ...
      await axios.delete(`${MEMORY_API_URL}/api/memory/short-term/${memory.id}`)
 ...
 */
export async function cleanupExpiredMemories(params: CleanupParams): Promise<ConsolidationResult> {
  const logger = Context.current().log
