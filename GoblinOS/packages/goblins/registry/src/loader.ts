...
async function resolveConfigPath(configPath?: string) {
  ...
  throw new GuildRegistryError('Unable to locate goblins.yaml configuration file')
}
...
function normalizeGuild(source: GuildRegistrySource['guilds'][number]): GuildWithLookups {
  const memberIds = new Set(source.members.map((member) => member.id))
  if (memberIds.size !== source.members.length) {
    throw new GuildRegistryError(`Duplicate member IDs detected in guild ${source.id}`)
  }

  const toolMap = new Map<string, GuildToolReference>()
  for (const tool of source.toolbelt) {
    if (toolMap.has(tool.id)) {
      throw new GuildRegistryError(`Duplicate tool id "${tool.id}" in guild ${source.id}`)
    }
  ...
  for (const tool of toolMap.values()) {
    ...
    const alreadyAssigned = owner.toolDetails.some((assigned) => assigned.id === tool.id)
    if (!alreadyAssigned) {
...
function parseRegistryContents(contents: string): GuildRegistry {
  ...
  const source = GuildRegistrySchema.parse(parsed)
...
export async function loadRegistry(options: LoadOptions = {}): Promise<GuildRegistry> {
