...
export const BrainConfigSchema = z.object({
  // Default LiteBrain routing for Forge Master (local -> cloud)
  memberId: z.string().default('dregg-embercode'),
  defaultModel: z.string().default('ollama'),
  fallbackModels: z.array(z.string()).default(['deepseek-r1']),
...
export class SmithyBrain {
  ...
  constructor(logger: Logger, config: Partial<BrainConfig> = {}) {
    ...
    this.provider = createProvider({
      ...
      telemetry: this.config.enableTracing,
      trackCost: true,
    })
  }
  ...
  private async generatePlan(request: BrainRequest): Promise<BrainResponse['plan']> {
    ...
    this.logger.debug?.('Generating plan with LiteBrain routing', {
      model: this.config.defaultModel,
    ...
    const response = await this.provider.chat({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are Smithy, an expert AI scaffolding assistant for the Forge Guild system.',
        },
...
Respond with valid JSON only:`
  ...
  /**
   * Check if LiteBrain provider is healthy
   */
  private async checkHealth(): Promise<void> {
    try {
      // Perform a lightweight, non-streaming, fast call to validate provider route
      const client = this.provider.getClient()
      const models = await client.models.list()
      const availableIds = new Set(
        (models.data || []).map((m: { id?: string; name?: string }) => m.id || m.name || '')
      )

      if (!availableIds.has(this.config.defaultModel)) {
        throw new Error(`Model ${this.config.defaultModel} not available on provider`)
      }

      this.config.fallbackModels.forEach((fallback) => {
        if (fallback && !availableIds.has(fallback)) {
          this.logger.warn?.('Fallback model missing from provider', { fallback })
        }
      })

      this.logger.debug?.('Provider health check passed', {
        baseURL: this.config.providerBaseURL,
        defaultModel: this.config.defaultModel,
        memberId: this.config.memberId,
      })
      ...
      throw new BrainError(
        `Provider unavailable: ${error instanceof Error ? error.message : String(error)}`,
