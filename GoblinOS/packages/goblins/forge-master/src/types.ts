 ...
 */
// NOTE: we used to re-export Overmind types here. During triage we provide
// permissive local fallbacks (defined below) to keep the build progressing.
// Replace with proper re-exports once package-level types are stable.
...
// Brain request type used by CLI and router
export interface BrainRequest {
  task: string
  context?: Record<string, unknown>
}

// Local fallbacks for quick triage: if Overmind types are not resolved
// via package shims, provide permissive fallbacks. Remove when upstream
// types are available.
