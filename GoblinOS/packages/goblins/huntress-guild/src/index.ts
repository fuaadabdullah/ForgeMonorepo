...
const guild = (() => {
  ...
  if (!value) {
    throw new GuildRegistryError('Huntress guild definition not found in goblins.yaml')
  }
  return value
})()
...
function getTool(toolId: string) {
  ...
  if (!tool) {
    throw new GuildRegistryError(`Tool ${toolId} is not registered for the Huntress guild`)
  }
  return tool
}
...
export function createProgram() {
  ...
  program
    .name('huntress-guild')
    .description('Huntress Guild CLI — Smoke probes, flaky extermination, and signal scouting')
    .showHelpAfterError()

  program
    .command('smoke')
    .description('Run platform smoke test (tools/smoke.sh) and report incidents')
    .action(async () => {
...
export async function run(argv: string[] = process.argv): Promise<void> {
  ...
  try {
    ...
    logger.error({ error: (error as Error).message }, 'Huntress guild command failed')
    process.exit(1)
  }
}
