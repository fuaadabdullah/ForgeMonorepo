...
const guild = (() => {
  ...
  if (!value) {
    throw new GuildRegistryError('Mages guild definition not found in goblins.yaml')
  }
  return value
})()
...
function getTool(toolId: string) {
  ...
  if (!tool) {
    throw new GuildRegistryError(`Tool ${toolId} is not registered for the Mages guild`)
  }
  return tool
}
...
export function createProgram() {
  ...
  program
    .name('mages-guild')
    .description('Mages Guild CLI — Forecasting, anomaly hunting, and quality gates')
    .showHelpAfterError()
  ...
  program
    ...
    .description('Alias for quality:lint (kept for parity with Overmind dashboards)')
    .action(async () => {
  ...
  program
    ...
    .description('Validate Forge knowledge vault integrity (tools/validate_forge_vault.sh)')
    .action(async () => {
...
export async function run(argv: string[] = process.argv): Promise<void> {
  ...
  try {
    ...
    logger.error({ error: (error as Error).message }, 'Mages guild command failed')
    process.exit(1)
  }
}
