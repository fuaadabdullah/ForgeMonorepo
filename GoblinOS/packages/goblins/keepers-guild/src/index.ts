...
const guild = (() => {
  ...
  if (!value) {
    throw new GuildRegistryError('Keepers guild definition not found in goblins.yaml')
  }
  return value
})()
...
function getTool(toolId: string) {
  ...
  if (!tool) {
    throw new GuildRegistryError(`Tool ${toolId} is not registered for the Keepers guild`)
  }
  return tool
}
...
export function createProgram() {
  ...
  program
    .name('keepers-guild')
    .description('Keepers Guild CLI — Secrets, SBOM integrity, and storage hygiene')
    .showHelpAfterError()
...
export async function run(argv: string[] = process.argv): Promise<void> {
  ...
  try {
    ...
    logger.error({ error: (error as Error).message }, 'Keepers guild command failed')
    process.exit(1)
  }
}
