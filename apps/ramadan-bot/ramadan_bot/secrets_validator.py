...
def validate_secrets() -> Tuple[bool, Dict[str, str]]:
    ...
    secrets_status = {
        "SILICONFLOW_API_KEY": "✅" if config.SILICONFLOW_API_KEY else "❌ Not configured",
        "SENDGRID_API_KEY": "✅" if config.SENDGRID_API_KEY else "⚠️ Optional (email disabled)",
        ...
        "LON": f"✅ {config.LON}",
        "TZ": f"✅ {config.TZ}",
        "SMS_RECIPIENTS": f"✅ {len(config.SMS_RECIPIENTS)} recipients" if config.SMS_RECIPIENTS else "⚠️ No SMS recipients",
    }
    
    # Check critical secrets
    critical_configured = bool(config.SILICONFLOW_API_KEY)
    optional_warning = not (config.SENDGRID_API_KEY or (config.EMAIL_USER and config.EMAIL_PASS))
...
def print_status():
    ...
    try:
        ...
        if not is_valid:
            ...
            print("   Preview generation will work, but Send features will be unavailable.\n")
            return True
        else:
