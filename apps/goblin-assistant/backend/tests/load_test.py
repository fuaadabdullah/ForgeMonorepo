...
class GoblinAssistantUser(HttpUser):
    ...
    def health_all(self):
        ...
        self.client.get("/v1/health/all")

    @task(5)
    def health_chroma(self):
        """Test ChromaDB health check."""
        self.client.get("/v1/health/chroma/status")

    @task(3)
    def health_sandbox(self):
        """Test sandbox health check."""
        self.client.get("/v1/health/sandbox/status")

    @task(2)
    def get_models(self):
        """Test available models endpoint."""
        headers = {"Authorization": f"Bearer {self.token}"} if self.token else {}
        self.client.get("/v1/chat/models", headers=headers)
    ...
    def get_settings(self):
        ...
        self.client.get("/v1/settings/", headers=headers)
...
def on_test_stop(environment, **kwargs):
    ...
    chat_stats = stats.get("/v1/chat/completions", "POST")
    ...
    auth_stats = stats.get("/v1/auth/login", "POST")
