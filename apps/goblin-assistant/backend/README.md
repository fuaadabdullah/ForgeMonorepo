...
- `GET /` - Root
- `GET /health` - Base ops health (top-level compatibility endpoint)
- `POST /v1/chat/completions` - Chat completions (intelligent routing)
- `POST /v1/debugger/suggest` - Debugger endpoint
- `POST /v1/parse` - Natural-language orchestration parser
- `POST /v1/execute` - Execute Goblin tasks
- `POST /v1/api/generate` - Provider-fallback chat generation endpoint
- `GET /v1/metrics` - Prometheus metrics

## Breaking Change Notice

- Canonical API surface is now versioned under `/v1/*`.
- Unversioned app endpoints have been removed.
- Top-level `GET /` and `GET /health` remain for operational compatibility.
...
# Canonical core suite (excludes optional/integration/e2e/slow)
