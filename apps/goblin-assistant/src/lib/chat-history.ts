...
const normalizeStoredMessage = (value: unknown, fallbackIndex: number): ChatMessage | null => {
  ...
  const raw = value as Partial<ChatMessage> & { role?: unknown; content?: unknown };
  ...
  return {
    id: typeof raw.id === 'string' && raw.id ? raw.id : `${createLocalId()}-${fallbackIndex}`,
...
export const readChatMessages = (conversationId: string): ChatMessage[] => {
  ...
  try {
    ...
    const parsed = JSON.parse(raw);
    if (!Array.isArray(parsed)) return [];
    return parsed.map((item, idx) => normalizeStoredMessage(item, idx)).filter(Boolean) as ChatMessage[];
  } catch (error) {
...
export const preloadRecentChat = (
  ...
  const payload = { threadId: thread.id, messages, timestamp: new Date().toISOString() };
  try {
...
export const readPreloadedChat = (): { threadId: string; messages: ChatMessage[] } | null => {
  ...
  try {
    ...
    return {
      ...
      messages: (parsed.messages as unknown[]).map((item, idx) => normalizeStoredMessage(item, idx)).filter(Boolean) as ChatMessage[],
    };
  } catch (error) {
