...
app.use((req, res, next) => {
  ...
  res.setHeader('Content-Security-Policy',
    ...
    "style-src 'self' 'unsafe-inline'; " +
    "connect-src 'self' https://goblin-backend.fly.dev;">
  );
...
[http_service]
  [http_service.headers]
    Content-Security-Policy = "default-src 'self'; script-src 'self' https://challenges.cloudflare.com; style-src 'self' 'unsafe-inline'; connect-src 'self' https://goblin-backend.fly.dev;"
    X-Content-Type-Options = "nosniff"
    X-Frame-Options = "DENY"
    Strict-Transport-Security = "max-age=31536000; includeSubDomains"
    Referrer-Policy = "strict-origin-when-cross-origin"
      ...
      "headers": [
        {
          ...
          "value": "default-src 'self'; script-src 'self' https://challenges.cloudflare.com; style-src 'self' 'unsafe-inline'; connect-src 'self' https://goblin-backend.fly.dev;">
        },
        {
